...
exports.parse = function parseIntelHex(data, bufferSize) {
	...
	var buf = new Buffer(bufferSize || 8192),
        ...
        let lowestAddress, highestAddress;
		...
		switch(recordType)
		{
			case DATA:
				var absoluteAddress = highAddress + lowAddress;
                                const absEnd = absoluteAddress + dataLength;
				...
				bufLength = Math.max(bufLength, absoluteAddress + dataLength);
                                if (lowestAddress == null || lowestAddress > absoluteAddress) {
                                  lowestAddress = absoluteAddress;
                                }
                                if (highestAddress == null || highestAddress < absEnd) {
                                  highestAddress = absEnd;
                                }
				break;
			case END_OF_FILE:
				if(dataLength != 0)
